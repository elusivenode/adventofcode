...
func main () {
	...
	minPos := 0
	for i := 0; i <= maxPos; i++ {
		calculateFuel(i, &crabPositions, &fuelCostMap, &stepCost)
...
func calculateFuel(alignmentPoint int, startingPositions *[]int, fuelCostMap *map[int]int, stepCost *map[int]int) {
	fuelCost := 0
	for _, c := range *startingPositions {
		steps := int(math.Abs(float64(alignmentPoint - c)))
		sc := *stepCost
		cost := sc[steps]
		fuelCost += cost
	}
