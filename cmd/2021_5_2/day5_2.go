	...
	for scanner.Scan() {
		...
		for i, p := range points {
			...
			if i == 0 {
				...
				newLine := newLine(x1, y1, x2, y2, isHor, isVert, isDiag)
...
func processLine(pointsMap map[string]int, line *line) {
	if line.isHor {
		...
		//sloping up
		if (line.x1 - line.x2) * (line.y1 - line.y2) > 1 {
			if (line.x1 == xMin) {
				yStart = line.y1
			} else {
				yStart = line.y2
			...
			for i := xMin; i <= xMax; i++ {
				point := strconv.Itoa(i) + "-" + strconv.Itoa(yStart)
				pointsMap[point]++
				yStart++
			}
		} else {  //sloping down
...
func newLine(x1 int, y1 int, x2 int, y2 int, isHor bool, isVert bool, isDiag bool) *line {
	newLine := line {
		x1: x1,
		y1: y1,
		x2: x2,
